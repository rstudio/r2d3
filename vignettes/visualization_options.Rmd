...
2) Specifying the type of HTML container to render content withing (e.g. "svg", "canvas", "div", etc.)
...
To match themes provided by RStudio and other runtime environments, **r2d3** provides a `theme` JavaScript object that contains colors for `background` and `foreground` that you can optionally use to make your visualization match your runtime.

For instance, to match the fill color of `rect` elements with the background we write a D3 script as follows:
...
Notice that **r2d3** assigns **background** to the `background` style and **foreground** to the `fill`, `stroke` and `color` styles. Other environments hosting **r2d3** can set the `r2d3.theme` option to customize the value of the `theme` object.
