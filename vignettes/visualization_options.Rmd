...
5) Making use of runtime themes

6) Customizing the type of HTML container
...
The `container` parameter can be further customized to support containers generated by a JavaScript code by setting the `r2d3.container.<name>` option to code that generates this HTML element. This can be helpful to set your own marging and other more advanced initialization patterns.

For instance, the following code overrides the default `svg` element with one that will always set the background color to 'coral':

```{r}
options("r2d3.container.svg" = "
  d3.create('svg')
    .attr('width', width)
    .attr('height', height)
