...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/population>
</div>
...
r2d3(data = read.csv("population.csv"), d3_version = 3, dependencies = "population.css", options = list(year = 2000), script = "population.js")
```

```{r, echo=FALSE}
r2d3(data = read.csv("population.csv"), d3_version = 3, dependencies = "population.css", options = list(year = 2000), script = "population.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("population.js", warn = FALSE))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("population.css", warn = FALSE))
cat("```\n")
```
...
$('a.list-group-item[href="../population/"]').addClass('active');
