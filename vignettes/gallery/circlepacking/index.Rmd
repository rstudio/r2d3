...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/circlepacking>
</div>
...
r2d3(data = jsonlite::read_json("flare.json"), d3_version = 4, dependencies = "circlepacking.css", script = "circlepacking.js")
```

```{r, echo=FALSE}
r2d3(data = jsonlite::read_json("flare.json"), d3_version = 4, dependencies = "circlepacking.css", script = "circlepacking.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("circlepacking.js", warn = FALSE))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("circlepacking.css", warn = FALSE))
cat("```\n")
```
...
$('a.list-group-item[href="../circlepacking/"]').addClass('active');
