...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/treemap>
</div>
...
r2d3(data = jsonlite::read_json("flare.json"), d3_version = 4, dependencies = "treemap.css", script = "treemap.js")
```

```{r, echo=FALSE}
r2d3(data = jsonlite::read_json("flare.json"), d3_version = 4, dependencies = "treemap.css", script = "treemap.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("treemap.js"))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("treemap.css"))
cat("```\n")
```
