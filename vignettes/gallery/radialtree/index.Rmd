...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/radialtree>
</div>
...
r2d3(data = read.csv("flare.csv"), d3_version = 4, dependencies = "radialtree.css", script = "radialtree.js")
```

```{r, echo=FALSE}
r2d3(data = read.csv("flare.csv"), d3_version = 4, dependencies = "radialtree.css", script = "radialtree.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("radialtree.js", warn = FALSE))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("radialtree.css", warn = FALSE))
cat("```\n")
```
...
$('a.list-group-item[href="../radialtree/"]').addClass('active');
