...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/voronoi>
</div>
...
r2d3(data = c(), d3_version = 4, dependencies = "voronoi.css", script = "voronoi.js")
```

```{r, echo=FALSE}
r2d3(data = c(), d3_version = 4, dependencies = "voronoi.css", script = "voronoi.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("voronoi.js", warn = FALSE))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("voronoi.css", warn = FALSE))
cat("```\n")
```
...
$('a.list-group-item[href="../voronoi/"]').addClass('active');
