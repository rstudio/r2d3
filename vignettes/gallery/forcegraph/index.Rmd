...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/forcegraph>
</div>
...
```{r, echo=FALSE}
r2d3(data = jsonlite::read_json("miserables.json"), d3_version = 4, script = "forcegraph.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("forcegraph.js", warn = FALSE))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("forcegraph.css", warn = FALSE))
cat("```\n")
```
...
$('a.list-group-item[href="../forcegraph/"]').addClass('active');
