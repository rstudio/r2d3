...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/stackedbars>
</div>
...
r2d3(data = c(), d3_version = 4, dependencies = "stackedbars.css", script = "stackedbars.js")
```

```{r, echo=FALSE}
r2d3(data = c(), d3_version = 4, dependencies = "stackedbars.css", script = "stackedbars.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("stackedbars.js", warn = FALSE))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("stackedbars.css", warn = FALSE))
cat("```\n")
```
...
$('a.list-group-item[href="../stackedbars/"]').addClass('active');
