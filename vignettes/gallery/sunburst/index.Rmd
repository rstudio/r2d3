...
<div id="source-link">
<https://github.com/rstudio/r2d3/tree/master/vignettes/gallery/sunburst>
</div>
...
r2d3(data = jsonlite::read_json("flare.json"), d3_version = 3, dependencies = "sunburst.css", script = "sunburst.js")
```

```{r, echo=FALSE}
r2d3(data = jsonlite::read_json("flare.json"), d3_version = 3, dependencies = "sunburst.css", script = "sunburst.js", width="100%")
```
...
```{r, results='asis'}
cat("```js\n")
writeLines(readLines("sunburst.js"))
cat("```\n")
```
...
```{r, results='asis'}
cat("```css\n")
writeLines(readLines("sunburst.css"))
cat("```\n")
```
