...
If an error occurs while rendering a D3 visualization using preview mode then the error will be displayed in the Viewer pane (overwriting any previous visualization output):
...
One way of troubleshooting D3 visualizations that is also common while troubleshooting JavaScript is by printing data to the JavaScript console. This is usually accomplished by calling `console.log()` with a message that can  help you troubleshoot the problem at hand.
...
We can change the script above into to print meaningful information to the console, in this case, we think the problem might be in the `width` so we print this value to the console as follows:
...
**r2d3** renders renders the JavaScript console output directly into the visualization to help you troubleshoot with ease. For this example, this will show that the `width` is `0.2` which is too small to be visible:
